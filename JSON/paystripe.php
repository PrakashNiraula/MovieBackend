...
$paymentmod = 'Stripe Payment-Gateway';
...
if (!empty($userid) && !empty($cartid) && !empty($paymentmod) && !empty($address) && !empty($phone) && !empty($total) && !empty($token)) {



/////Initialising keys etc ////////////////////////////////////////////////////////
        ...
        if ($status == 'ACTIVE') {
            ...
            if ($rowcountcart > 0) {
                ...
                if ($chargeJson['amount_refunded'] == 0 && empty($chargeJson['failure_code']) && $chargeJson['paid'] == 1 && $chargeJson['captured'] == 1) {
                    ...
                    $paymentref = $chargeJson['balance_transaction'];
...
VALUES('" . $orderid . "','" . $paymentref . "','" . $paymentmod . "','" . $payment_status . "','" . $address . "','" . $date . "','" . $order_status . "','" . $phone . "','" . $total . "','" . $uid . "','" . $shipping . "','" . $tax . "')";
                      ...
                      <table cellspacing="0" cellpadding="0" width="100%">
                        <tr>
                          ...
                          <td class="product">
                            ...
                            $message .=' ' . strip_tags($product_name) . '  WITH </span> <br />
                      ...
                      <table cellspacing="0" cellpadding="0" width="100%">
                        <tr>
                          ...
                          <td class="product">
                            ...
                            $message .=' ' . strip_tags($var['product_name']) . '  WITH </span> <br />
...
/////////////////////////
                    ...
                    if (!$mail->Send()) {
                       ...
                       $minfo = array("success"=>'true', "message"=>'Order placed successfully ',"amount"=> $amount,"status"=>$payment_status,"date"=>$date,"paymentref"=>$paymentref,"currency"=>$currency,"mode"=>$paymentmod);
      $jsondata = json_encode($minfo); print_r($jsondata);  mysqli_close($conn); exit();
                    }
                } else {
                    $minfo = array("success" => 'false', "message" => 'Payment fail');
                ...
                $minfo = array("success" => 'false', "message" => 'Your cart is empty');
                $jsondata = json_encode($minfo);
                print_r($jsondata);
                mysqli_close($conn);
                exit();
